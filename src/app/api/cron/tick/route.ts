 ...
 *
 * 하는 일(순서가 중요하다 — 아래 참고):
 *  1. 04·10·17·21시(KST) 슬롯 — "지나간 슬롯 중 아직 안 보낸 것"을 보낸다.
 ...
 *  2. 03·15시 — 최근 실거래를 갱신한다 (오늘의 요약 페이지가 최신 데이터를 쓰도록)
 *  3. 매시간 — AI 시황·정책 요약을 미리 만들어 캐시에 넣는다
 *
 * ── 왜 발송이 맨 앞인가 ───────────────────────────────────────────
 * 예전에는 갱신 → AI 요약 → 정책 요약 → 발송 순서였다. AI 요약은 사용자 수만큼
 * 대시보드를 만들고 OpenAI 를 부르느라 10~15분이 걸렸고, 그동안 발송이 통째로
 * 밀렸다 — 05시 브리핑이 05:31에 도착하던 실제 원인이다. 발송은 사람이 기다리는
 * 일이고 캐시 예열은 아니므로, 사람 몫을 먼저 끝내고 나머지를 뒤로 돌린다.
 *
 * 수동 실행:
...
export async function GET(request: Request) {
  ...
  try {
    ...
    /* 0) 사용자 설정 — 발송에도, 갱신 대상 지역 계산에도 쓴다.
    ...
    /* 1) 발송 — 사람이 기다리는 일이므로 가장 먼저 한다.
       사용자마다 자기 설정으로 만든 브리핑을 자기 수신자에게 보낸다.
       채널별 발송 주기가 다르다:
        - 텔레그램: 슬롯(04·10·17·21시) 4회, 브리핑 전문
    ...
    const willSend = !refreshOnly && Boolean(slot);
    ...
    /* 2) 최근 실거래 갱신 — 12시간에 1번(03·15시 KST)만.
       실거래는 하루 두 번이면 충분하다 (신고 기한 30일, 당일 체결 즉시 반영도 아님).
       뉴스·AI 요약은 아래에서 매시간 그대로 돈다.

       ── 왜 발송 슬롯(04·10·17·21시)을 피하는가 ────────────────────
       갱신은 최대 180초가 걸린다. 발송과 같은 실행에 몰리면 GitHub Actions 의
       응답 대기(290초)를 넘겨 실패로 잡힌다. 발송 한 시간 전에 끝내 둔다. */
    ...
    /* 4) 최신 부동산 정책 요약 — 전역 1건. 정책은 사용자별로 다르지 않다.
