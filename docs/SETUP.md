...
| 🟠 권장     | `NEXT_PUBLIC_KAKAO_JS_KEY`                          | 동 클릭 시 실제 지도 + 단지 마커       | 위와 동일 | 무료    |
...
### 6-3. 카카오맵 활성화 (주변 입지 + 실제 지도)

1. **제품 설정 → 카카오맵** → 활성화 **ON**
   - 이걸 켜야 단지 주변 지하철역·학교·마트 정보가 나옵니다.
2. **앱 키 → JavaScript 키** 복사 (REST API 키와 **다른 키**입니다)
   ```
   NEXT_PUBLIC_KAKAO_JS_KEY=<JavaScript 키>
   ```
   동을 클릭했을 때 **실제 지도 위에 아파트 단지 마커**가 뜹니다. 없으면 목록만 나옵니다.
3. **앱 설정 → 플랫폼 → Web** 에 사이트 도메인 등록 (둘 다)
   ```
   http://localhost:3000
   https://<배포주소>
   ```
...
> **왜 네이버 부동산이 아닌가**: 네이버는 부동산 매물 API 를 공개하지 않고, 크롤링은
> 이용약관 위반이며 실제 소송 사례도 있습니다. 이 앱은 **국토교통부 실거래가(실제 체결가)** 를
> 쓰고, 호가는 설정에서 직접 입력받습니다. 호가를 API 로 여는 국내 플랫폼은 없습니다.
